...
for i in 0...data.shape[0] - LENGTH_OF_SEQUENCE - LENGTH_OF_VERIFI
	sequences << ((data[i...i + LENGTH_OF_SEQUENCE, true] / data[i + LENGTH_OF_SEQUENCE - 1, 3] - 1.0) * 5).to_a
	...
	min = [data[i + LENGTH_OF_SEQUENCE..i + LENGTH_OF_SEQUENCE + LENGTH_OF_VERIFI, 2].min, data[i + LENGTH_OF_SEQUENCE - 1, 3]].min \
			- data[i + LENGTH_OF_SEQUENCE - 1, 3]
    if max.abs > min.abs
    	min == 0 ? v = max * 10 : v = (max/min).abs * max / 10
    	v = 10 if v > 10
    else
    	max == 0 ? v = min * 10 : v = (min/max).abs * min / 10
    	v = -10 if v < -10
    end
    t << v
end
...
# 解析実行
lstm.train(train_data, test_data, 200, 1, 4, 10, 1)
#=end
