...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(2) as pool:
    	for epoch in range(TRAINING_EPOCHS):
            ...
            s1 = sequences[:len(sequences)//2]
            s2 = sequences[len(sequences)//2:]
            t1 = t[:len(sequences)//2]
            t2 = t[len(sequences)//2:]
            
#            loss = model(sequences, Variable(t[:, np.newaxis]))
            
            loss1 = pool.apply_async(model, tuple([s1,Variable(t1[:, np.newaxis])]))
            loss2 = pool.apply_async(model, tuple([s2,Variable(t2[:, np.newaxis])]))
            pool.close()
            pool.join()
            loss = loss1.get() + loss2.get()
